...
def ran(v, r = 5.0):
    return v + (np.random.random(v.shape) - 0.5) * 2. * r
...
#    multidrawHess(xyz, hess, mass, VAR, template, portlist, state_templates=state_templates)
    ...
    RES = [(np.random.random(VAR.shape) * 2. - 1.) * 400.0 for _ in range(NSAMPLE)]
    for ni in range(NROUND):
        ...
        RES = [i[1] for i in score_list[:REMAIN]]
        for j in range(len(RES), NSAMPLE):
            RES.append(ran(mult(np.random.choice(score_list[:REMAIN]),np.random.choice(score_list[:REMAIN]))))
